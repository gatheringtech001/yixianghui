...
- `sources[].evidence`：1–4条支持答案的原文短句，每条2–350字符。服务端逐字验证它们存在于该来源的 `quote` 中，拒绝模型改写或伪造的摘录。正文中的URL查询参数在进入模型和引用输出前移除，原件访问以结构化媒体信息为准。
...
图片、照片、原图或视频查询会按对应 `media.kind` 过滤混合召回，两种媒体同时出现时检索两者。正文中的图片文件名不会作为媒体命中。没有合适媒体时明确拒答，不退回普通文档冒充原图。`evidence` 逐字校验也不等于语义事实审核，来源中的营销医疗说法不能当成医学结论。
