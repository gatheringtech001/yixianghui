...
- `sources[].evidence`：1–4个支持答案的原文片段，每段2–350字符。服务端预先给来源片段编号，模型仅选择编号，程序直接提取原文，拒绝不存在或重复的编号，不让模型抄写证据。正文中的URL查询参数在进入模型和引用输出前移除，原件访问以结构化媒体信息为准。
...
服务执行问题 embedding、Qdrant 混合召回，再由**一次 Luna 调用完成资料筛选和回答**，不会额外调用独立 rerank。明确编号基地会依据已索引文档目录分别检索，多对象和混合媒体查询按对象/类型均衡合并，最多送入30个候选。单次最多比较4个已识别的编号基地，超过返回400。
模型从统一的来源列表及编号片段中选择证据，再输出与来源列表下标绑定的段落；回答中的 `[S1]` 等标记由程序生成，不由模型自由拼写。未知下标、无效片段或超出引用预算仍显式失败。公共响应仍为 `answer` 字符串和 `sources` 数组，调用方不需要处理内部段落结构。
`retrievalCoverage` 返回各对象/媒体类型本次候选数，`exhaustive=false` 表示不是全库盘点，不能用它推断资料或素材的总量。
对识别出的明确天数/年龄边界，如果相邻条款“以内/以上”或年龄段存在交叠，程序直接并列原文并说明不能确定，`reason=ambiguous_policy_boundary`；不调用模型选择收费或退改标准。宠物政策未检索到时返回 `grounded=false`、`reason=pet_policy_not_retrieved`，不以日常保洁费替代。此类程序回答无Luna用量字段。
这些校验不能自动证明所有语义推断都正确，重要业务结论仍需核实；源文里的边界及价格冲突没有被自动改写。
图片、照片、原图或视频查询会按对应 `media.kind` 过滤混合召回，两种媒体同时出现时检索两者。正文中的图片文件名不会作为媒体命中。识别到候选标题中的明确编号基地（如弥勒二号、普洱一号）时，问答仅将对应对象送入模型；比较多个明确基地时保留多个对象。没有合适媒体时明确拒答，不退回普通文档冒充原图。`evidence` 原文提取也不等于语义事实审核，来源中的营销医疗说法不能当成医学结论。
价格、库存、上下架状态来自同步快照，不能作为实时下单承诺。接口当前为单轮问答，不接收会话历史。

### 重复回归测试

`node --env-file=<私有连接文件> knowledge-service/evaluate.mjs 2 <本地结果.json>`

运行16个固定场景、每场景重复2轮，仅调用读取型问答接口，不写入知识库。覆盖价格、取消政策、原文冲突、已知图片、缺失图片、视频时间段、医疗营销、实时房态、未知基地、指令注入和指代不明的追问。接口没有会话记忆，追问必须补全对象名称；不能从上次请求隐式获取上下文。结果记录答案、引用、耗时和规则检查；规则通过率不是准确率，必须独立对照来源评审。遇429停止，不自动重复计费请求。
...
- `400`：问答请求 JSON 或参数无效。
- `401`：查询令牌缺失或错误。
- `403`：问答/检索接口使用了 POST 以外的方法，或访问了未开放的操作；素材索引接口仅开放 GET/PUT。
