		...
		<view class="u-datetime-picker">
			...
			<view class="u-picker-body">
				...
				>
					...
					<picker-view-column v-if="showColumnCom && params.city" :key="'city-' + province">
						<view class="u-column-item" v-for="(item, index) in citys" :key="index">
							<view class="u-line-1">{{ item.name }}</view>
						</view>
					</picker-view-column>
					<picker-view-column v-if="showColumnCom && params.area" :key="'area-' + province + '-' + city">
...
export default {
	...
	data() {
		return {
			...
			valueArr: [],
      provinces: provinces,
      citys: (citys && citys.length > 0) ? citys[0] : [],
      areas: (areas && areas.length > 0 && areas[0] && areas[0].length > 0) ? areas[0][0] : [],
			province: 0,
			city: 0,
			area: 0,
	...
	watch: {
		...
		regionChange(val) {
      if (this.province >= 0 && this.province < citys.length && citys[this.province]) {
        this.citys = citys[this.province];
      } else {
        this.citys = (citys && citys.length > 0) ? citys[0] : [];
        this.city = 0;
      }

      if (this.province >= 0 && this.province < areas.length &&
          this.city >= 0 && this.city < areas[this.province].length &&
          areas[this.province][this.city]) {
        this.areas = areas[this.province][this.city];
      } else {
        this.areas = (areas && areas.length > 0 && areas[0] && areas[0].length > 0) ? areas[0][0] : [];
        this.area = 0;
      }

      if (this.mode === 'region' && Array.isArray(this.valueArr) && this.valueArr.length >= 3) {
	...
	methods: {
      ...
      if (this.areaCode.length) {
        const targetCode = this.areaCode[0];
        useCode = true;
        selectedIndex = provinces.findIndex(province => province.code == targetCode);
      } else if (this.defaultRegion.length) {
        const targetName = this.defaultRegion[0];
        selectedIndex = provinces.findIndex(province => province.name == targetName);
      }

      // 确保索引有效
      this.province = selectedIndex >= 0 ? selectedIndex : 0;
      ...
      // 边界检查
      if (!citys[this.province]) {
        this.city = 0;
        this.citys = [];
        this.valueArr.splice(1, 1, 0);
        return;
      }
      ...
      if (this.areaCode.length > 1) {
        const targetCode = this.areaCode[1];
        useCode = true;
        selectedIndex = citys[this.province].findIndex(city => city.code == targetCode);
      } else if (this.defaultRegion.length > 1) {
        const targetName = this.defaultRegion[1];
        selectedIndex = citys[this.province].findIndex(city => city.name == targetName);
      }

      this.city = selectedIndex >= 0 ? selectedIndex : 0;
      this.citys = citys[this.province];
      ...
      if (!areas || areas.length === 0) {
        console.warn('Areas data is empty');
        this.area = 0;
        this.areas = [];
        this.valueArr.splice(2, 1, 0);
        return;
      }

      if (this.province < 0 || this.province >= areas.length) {
        console.warn('Province index out of bounds:', this.province);
        this.area = 0;
        this.areas = [];
        this.valueArr.splice(2, 1, 0);
        return;
      ...
      if (!areas[this.province] || areas[this.province].length === 0) {
        console.warn('City data for province index is empty:', this.province);
        this.area = 0;
        this.areas = [];
        this.valueArr.splice(2, 1, 0);
        return;
      ...
      if (this.city < 0 || this.city >= areas[this.province].length) {
        console.warn('City index out of bounds:', this.city, 'for province:', this.province);
        this.area = 0;
        this.areas = [];
        this.valueArr.splice(2, 1, 0);
        return;
      ...
      // 现在可以安全访问 areas[this.province][this.city]
      let selectedIndex = 0;
      let useCode = false;

      if (this.areaCode.length > 2) {
        const targetCode = this.areaCode[2];
        useCode = true;
        selectedIndex = areas[this.province][this.city].findIndex(area => area.code == targetCode);
      } else if (this.defaultRegion.length > 2) {
        const targetName = this.defaultRegion[2];
        selectedIndex = areas[this.province][this.city].findIndex(area => area.name == targetName);
      }

      this.area = selectedIndex >= 0 ? selectedIndex : 0;
      this.areas = areas[this.province][this.city];
      this.valueArr.splice(2, 1, this.area);
		...
		// 用户更改picker的列选项
		change(e) {
      const oldValueArr = [...(this.valueArr || [])];
      const newValueArr = [...(e.detail.value || [])];

      if (this.mode == "region") {
        let provinceIndex = this.province;
        let cityIndex = this.city;
        let areaIndex = this.area;

        if (this.params.province) {
          provinceIndex = newValueArr[0] >= 0 ? newValueArr[0] : 0;
        }
        if (this.params.city) {
          cityIndex = newValueArr[1] >= 0 ? newValueArr[1] : 0;
        }
        if (this.params.area) {
          areaIndex = newValueArr[2] >= 0 ? newValueArr[2] : 0;
        }

        if (this.params.province && oldValueArr[0] !== newValueArr[0]) {
          cityIndex = 0;
          areaIndex = 0;
        } else if (this.params.city && oldValueArr[1] !== newValueArr[1]) {
          areaIndex = 0;
        }
        ...
        this.city = cityIndex;
        this.area = areaIndex;
        this.valueArr = [provinceIndex, cityIndex, areaIndex];
        return;
      }
      ...
      // #ifdef MP-WEIXIN
      if (this.moving) {
        console.log('Still moving, skip confirm'); // 添加日志以便调试
        return;
      }
      // #endif
      ...
      if (this.mode == "time") {
        ...
        console.log('Region data:', { // 添加调试日志
          province: this.province,
          city: this.city,
          area: this.area,
          provincesLength: this.provinces.length,
          citysLength: this.citys.length,
          areasLength: this.areas.length
        });

        if (this.params.province && this.province >= 0 && this.province < this.provinces.length) {
          result.province = this.provinces[this.province];
        }

        if (this.params.city &&
            this.province >= 0 && this.province < citys.length &&
            this.city >= 0 && this.city < citys[this.province].length) {
          result.city = citys[this.province][this.city];
        }

        if (this.params.area &&
            this.province >= 0 && this.province < areas.length &&
            this.city >= 0 && this.city < areas[this.province].length &&
            this.area >= 0 && this.area < areas[this.province][this.city].length) {
      ...
      console.log('Result to emit:', result); // 添加调试日志
