					...
					<ul class="each-month">
						<li class="each-day" v-for="(day,idx) in item.dayList" :key="idx"
							...
							<div :class="[addClassName(day, item.month, item.year)]"
								...
								<p class="price" v-if="day && price">{{price}}</p>
	...
	export default {
		...
		methods: {
			...
			createClendar() {
				...
				// 仅 H5 滚到起始月；小程序无 document，直接访问会抛错导致页面卡死
				// #ifdef H5
			...
			chooseDate(day, month, year) {
				...
				if (this.fixedEnd && this.startDates && !this.endDates) {
					this.$emit("calendarClick", choose);
					// this.endDates = this.resetTime(this.endDate);
				}
				if (this.mode == 1) {
...
<style lang="less" scoped>
	...
	.calendar-tz {
		width: 100%;
		/* 弹层内嵌：高度随月份内容撑开，由外层 scroll-view 滚动 */
		height: auto;
		min-height: 0;
		background: #fff;
		display: -webkit-box;
