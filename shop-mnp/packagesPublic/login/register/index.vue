...
export default {
	...
	onShow() {
		...
		if (userToken && userToken != '') {
			uni.switchTab({
				url: '/packagesPublic/member/index/index'
			});
			return true;
		}
		_this.timer = parseInt(_this.$db.get('timer'));
	...
	methods: {
				...
				this.$api.smsLogin(data, res => {
					if (res.status) {
						...
						this.$common.successToShow('注册成功', () => {
							...
							let redirect = this.$store.state.redirectPage ? this.$store.state.redirectPage : '/pages/my/index';
							this.$store.commit({
								type: 'redirect',
								page: ''
							});
		...
		toLogin() {
			this.$common.navigateTo('/packagesPublic/login/login/index1');
		},
		// 跳转到用户协议
		goAgreement() {
			let articleId = this.$store.state.config.user_agreement_id;
			this.$common.navigateTo('/packagesPublic/article/index?id_type=1&id=' + articleId);
		},
		// 跳转到隐私政策
		goPrivacy() {
