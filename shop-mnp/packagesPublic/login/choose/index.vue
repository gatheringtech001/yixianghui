  ...
  methods: {
      ...
      _this.$api.login2(data, function(res) {
        if (res.status) {
          ...
          if (typeof res.data.token == 'undefined') {
            uni.redirectTo({
              url: '/packagesPublic/login/login/index?user_wx_id=' + res.data.user_wx_id
            })
          } else {
            //登陆成功，设置token，并返回上一页
		...
		this.$api.alilogin1(data, res => {
			...
			if (res.status) {
				...
				if (!res.data.hasOwnProperty('token')) {
					this.$common.redirectTo('/packagesPublic/login/login/index?user_wx_id=' + res.data.user_wx_id);
				} else {
	...
	ttLoginStep(code, userInfo) {
		...
		this.$api.ttlogin(data, res => {
			if (res.status) {
				if (!res.data.hasOwnProperty('token')) {
					this.$common.redirectTo('/packagesPublic/login/login/index?user_wx_id=' + res.data.user_id);
				} else {
					this.$db.set('userToken', res.data.token)
					uni.navigateBack({
						delta: 1
