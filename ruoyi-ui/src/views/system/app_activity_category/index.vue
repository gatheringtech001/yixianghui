<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:app_activity_category:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
    ...
    <el-table
      v-if="refreshTable"
      v-loading="loading"
      :data="app_activity_categoryList"
      row-key="categoryId"
...
<!--      <el-table-column label="祖级id集合" align="center" prop="parentIds" />-->
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button
            ...
            v-hasPermi="['system:app_activity_category:edit']"
          >修改</el-button>
          <el-button
            size="mini"
            type="text"
          ...
          <el-button
            ...
            v-hasPermi="['system:app_activity_category:remove']"
          >删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="所属上级" prop="parentId">
          <treeselect v-model="form.parentId" :options="app_activity_categoryOptions" :normalizer="normalizer" placeholder="请选择所属上级" />
        </el-form-item>
        <el-form-item label="祖级id集合" prop="parentIds">
...
<script>
import { listApp_activity_category, getApp_activity_category, delApp_activity_category, addApp_activity_category, updateApp_activity_category } from "@/api/system/app_activity_category";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listApp_activity_category(this.queryParams).then(response => {
        this.app_activity_categoryList = this.handleTree(response.data, "categoryId", "parentId");
        this.loading = false;
      });
    },
    /** 转换活动分类数据结构 */
    ...
    handleUpdate(row) {
      ...
      getApp_activity_category(row.categoryId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.categoryId != null) {
            updateApp_activity_category(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addApp_activity_category(this.form).then(response => {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      this.$modal.confirm('是否确认删除活动分类编号为"' + row.categoryId + '"的数据项？').then(function() {
        return delApp_activity_category(row.categoryId);
      }).then(() => {
        this.getList();
