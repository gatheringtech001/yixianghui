...
<!--        <el-input-->
<!--          v-model="queryParams.gold"-->
<!--          placeholder="请输入金币数量"-->
<!--          clearable-->
<!--          @keyup.enter.native="handleQuery"-->
<!--        />-->
<!--      </el-form-item>-->
<!--      <el-form-item label="金币余额" prop="balance">-->
<!--        <el-input-->
...
<!--      </el-form-item>-->
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:app_user_gold_log:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:app_user_gold_log:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:app_user_gold_log:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:app_user_gold_log:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="app_user_gold_logList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="记录id" align="center" prop="logId" />
      <el-table-column label="用户id" align="center" prop="userId" />
      <el-table-column label="金币数量" align="center" prop="gold" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="金币数量" prop="gold">
          <el-input v-model="form.gold" placeholder="请输入金币数量" />
        </el-form-item>
        <el-form-item label="金币余额" prop="balance">
          <el-input v-model="form.balance" placeholder="请输入金币余额" />
        </el-form-item>
        <el-form-item label="变化类型" prop="tradeType">
...
<script>
import { listApp_user_gold_log, getApp_user_gold_log, delApp_user_gold_log, addApp_user_gold_log, updateApp_user_gold_log } from "@/api/system/app_user_gold_log";

export default {
  name: "App_user_gold_log",
  dicts: ['common_is_not', 'wallet_change_type'],
  data() {
    return {
      // 遮罩层
      loading: true,
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listApp_user_gold_log(this.queryParams).then(response => {
        this.app_user_gold_logList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getApp_user_gold_log(logId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.logId != null) {
            updateApp_user_gold_log(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addApp_user_gold_log(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除金币记录编号为"' + logIds + '"的数据项？').then(function() {
        return delApp_user_gold_log(logIds);
      }).then(() => {
        this.getList();
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/app_user_gold_log/export', {
        ...this.queryParams
      }, `app_user_gold_log_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
